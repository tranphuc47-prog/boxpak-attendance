...
function downloadCsv(rows,from,to){const head=["Ngày","MSNV","Họ tên","Bộ phận","Giờ vào","Giờ ra","Tổng số giờ phút có mặt"];const lines=[head,...rows.map(r=>[r.work_date,r.employee_code,r.full_name,r.department,timeText(r.first_seen),timeText(r.last_seen),durationText(r.first_seen,r.last_seen)])];const csv="\ufeff"+lines.map(row=>row.map(v=>`"${String(v??"").replaceAll('"','""')}"`).join(",")).join("\r\n");const a=document.createElement("a");a.href=URL.createObjectURL(new Blob([csv],{type:"text/csv;charset=utf-8"}));a.download=`cham-cong-${$("siteCode").value}-${from}-den-${to}.csv`;a.click();URL.revokeObjectURL(a.href);}
function openExport(){const selected=$("workDate").value||today();$("exportFrom").value=selected;$("exportTo").value=selected;$("exportStatus").textContent="";$("exportDialog").showModal();}
async function exportRange(){const from=$("exportFrom").value,to=$("exportTo").value,site=$("siteCode").value.trim();if(!from||!to){$("exportStatus").textContent="Hãy chọn đủ từ ngày và đến ngày.";return;}if(from>to){$("exportStatus").textContent="Từ ngày không được sau đến ngày.";return;}$("exportStatus").textContent="Đang chuẩn bị dữ liệu...";const {data,error}=await client.from("attendance_daily").select("*").eq("site_code",site).gte("work_date",from).lte("work_date",to).order("work_date").order("employee_code");if(error){$("exportStatus").textContent="Lỗi: "+error.message;return;}if(!data?.length){$("exportStatus").textContent="Không có dữ liệu trong khoảng ngày đã chọn.";return;}downloadCsv(data,from,to);$("exportDialog").close();}
...
$("logoutBtn").onclick=async()=>{await client.auth.signOut();showLogin();};$("refreshBtn").onclick=loadData;$("exportBtn").onclick=openExport;$("downloadRangeBtn").onclick=exportRange;$("workDate").onchange=loadData;
